 ...
 * 7. Improved UX copy and hint text
 *
 * Original features preserved:
 * - filterOptions / filter chips
 * - searchQuery / matchesSearch
 * - allItems / matchedItems
 * - openItemOnMap()
 * - handleMapReady() + AdvancedMarkerElement
 * - handleLocate() / Show my location
 * - selected info card with View Details + Google Maps links
 * - same-coordinate marker spread logic
 * - use_manual_coordinates hybrid strategy
 */
...
function TypeBadge({ type }: { type: MarkerType }) {
  ...
  return (
    <span
      className="inline-flex shrink-0 items-center px-1.5 py-0.5 rounded text-[10px] font-bold text-white leading-none"
      style={{ backgroundColor: markerColors[type] }}
    >
      {type === "office" ? t("Office", "行政單位") : t("Dept", "系所")}
    </span>
  );
}

/** Legend card overlaid on top-left of the map. pointer-events-none so it doesn't block map. */
function MapLegend() {
  const { t } = useLanguage();
  return (
    <div className="absolute top-3 left-3 z-10 pointer-events-none bg-white/90 backdrop-blur-sm border border-border rounded-lg shadow-sm px-3 py-2.5 space-y-2">
      ...
      <div className="shrink-0 bg-navy/[0.03] border-b border-border">
        <div className="container py-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display font-bold text-lg text-navy leading-tight">
                {t("Campus Map", "校園地圖")}
              </h1>
              <p className="text-xs text-muted-foreground hidden sm:block mt-0.5">
                {t(
                  "Select a category or search for an office, department, building, or service.",
                  "可先選擇分類，或搜尋行政單位、系所、建築物與服務。",
                )}
              </p>
            </div>
          </div>
        ...
        <div className="relative order-1 min-h-[55vh] lg:col-start-2 lg:row-start-1 lg:h-full lg:min-h-0">
          ...
          <div className="absolute top-3 right-3 z-10 flex flex-col items-end gap-2">
            ...
            <button
              ...
              className="bg-white border border-gray-200 rounded-full shadow-md p-2.5 hover:bg-gray-50 transition disabled:opacity-60"
            >
              {locating ? (
            ...
            {matchedItems.length === 0 ? (
              <div className="px-4 py-10 text-center">
                ...
                <p className="text-sm font-medium text-muted-foreground">
                  {t("No matching locations found.", "找不到符合條件的地點。")}
                </p>

...
export default function Navigation() {
  ...
  const scenarios_en = isOffice && 'common_scenarios_en' in item ? item.common_scenarios_en : "";
  const scenarios_zh = isOffice && 'common_scenarios_zh' in item ? item.common_scenarios_zh : "";
      ...
      <div className="bg-navy text-white">
        <div className="container py-5 md:py-6">
          <div className="flex items-center gap-3 mb-3">
            <button onClick={() => window.history.back()} className="text-white/70 hover:text-amber transition-colors">
          ...
          {college_en && (
            <p className="text-amber text-sm mt-1">{t(college_en, college_zh)}</p>
          )}
          {needs_review && (
          ...
          <div className="lg:col-span-3 space-y-5">
            ...
            <div className="bg-card rounded-xl border border-border p-5">
              <h2 className="font-display font-bold text-base text-navy mb-2">
                {isOffice ? t("What is this office for?", "這個單位做什麼？") : t("About this department", "關於此系所")}
              </h2>
              <p className="text-sm text-foreground/80 leading-relaxed">{t(func_en, func_zh)}</p>
            </div>
            ...
            {service_en && (
              <div className="bg-card rounded-xl border border-border p-5">
                ...
                <p className="text-sm text-foreground/80 leading-relaxed">{t(service_en, service_zh)}</p>
              </div>
            )}
            ...
            {scenarios_en && (
              <div className="bg-card rounded-xl border border-border p-5">
                ...
                <p className="text-sm text-foreground/80 leading-relaxed">{t(scenarios_en, scenarios_zh)}</p>
              </div>
            )}
                      ...
                      <div className="space-y-1.5">
                        {task.steps.map((step, i) => (
                          <div key={i} className="flex items-start gap-2">
                            ...
                            <p className="text-xs text-foreground/70 leading-relaxed">{t(step.en, step.zh)}</p>
                          </div>
                        ))}
                      </div>
                      {task.target_unit_id && (
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-card rounded-xl border border-border p-5">
              ...
              <div className="space-y-2.5">
                ...
                {email && (
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <a href={`mailto:${email}`} className="underline hover:text-navy">{email}</a>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(google_query)}`}
                  target="_blank"
